...
class NetworkInterfaceExplore:
    ...
    def __init__(self, game_mode: GameModeConfig, network: NetworkConfig,
        ...
        self.extension = extension  # adding the extension to explore isolation and compromised effects
        self.num_effect = num_extension  # add to how many nodes we need to modify
        ...
        if self.extension == 'standard':
        ...
        elif self.extension == 'isolate':
            status_nodes = [False for _ in range(len(nodes))]  # set all to false
            random_nodes = random.sample(range(0, len(nodes)), self.num_effect)

            for inode in random_nodes:
                status_nodes[inode] = True  # fix that
            ...
            self.initial_network_variables = {
                i: {
                    ...
                    "deceptive_node": False,
                    "blue_knows_intrusion": False,
                    "isolated": status_nodes[index],

                }
                for index, i in enumerate(nodes)
            }
        elif self.extension == 'compromise':
            status_nodes = [0 for _ in range(len(nodes))]  # set all to false
            random_nodes = random.sample(range(0, len(nodes)), self.num_effect)

            for inode in random_nodes:
                status_nodes[inode] = 1  # fix that

            self.initial_network_variables = {
                i: {
                    "vulnerability_score": 0,
                    "true_compromised_status": status_nodes[index],
                    "blue_view_compromised_status": 0,
                    "node_position": 0,
                    "deceptive_node": False,
                    "blue_knows_intrusion": False,
        ...
        elif self.extension == 'mix':
            # mix effect of both isolation and compromised
            compromised_nodes = [0 for _ in range(len(nodes))]  # set all to false
            isolation_nodes = [False for _ in range(len(nodes))]
            random_nodes_isolation = random.sample(range(0, len(nodes)), self.num_effect)
            random_nodes_compromise = random.sample(range(0, len(nodes)), self.num_effect)

            for index in range(self.num_effect):
                isolation_nodes[random_nodes_isolation[index]] = True  # fix that
                compromised_nodes[random_nodes_compromise[index]] = 1

            self.initial_network_variables = {
                i: {
                    "vulnerability_score": 0,
                    "true_compromised_status": compromised_nodes[index],
                    "blue_view_compromised_status": 0,
                    "node_position": 0,
                    "deceptive_node": False,
                    "blue_knows_intrusion": False,
                    "isolated": isolation_nodes[index],
                }
                for index, i in enumerate(nodes)
            }
        ...
        for inode in self.current_network_variables.keys():
            if self.current_network_variables[inode]['isolated']:
                self.isolate_node(inode)
