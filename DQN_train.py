...
from stable_baselines3.dqn import MlpPolicy as policy # policies
# from stable_baselines3.dqn import CnnPolicy as policy # policies
# from stable_baselines3.dqn import MultiInputPolicy as policy # policies
...
ntws = glob.glob(network_dir + 'nodes_60_training.pkl')
if len(ntws) == 1:
    matrix, positions = gtn.load(ntws[0])

else:
    matrix, positions = gtn.create_network(
        n_nodes = 100 , connectivity=0.3,
        output_dir = network_dir, filename = 'common_network_100',
        save_matrix = True,
        save_graph = False)
...
# setup for the loops
dir_agent = 'DQN/' #, 'A2C/', 'DQN/']

#names = ['DQN_std_100_nodes_mlp', 'DQN_lr_001_100_nodes_mlp', 'DQN_df_075_100_nodes_mlp']
#names = ['DQN_std_18_nodes_cnn', 'DQN_lr_001_18_nodes_cnn', 'DQN_df_075_18_nodes_cnn']
#names = ['DQN_std_50_nodes_ml', 'DQN_lr_001_50_nodes_ml', 'DQN_df_075_50_nodes_ml']
#names = ['DQN_std_18_nodes_ml', 'DQN_lr_001_18_nodes_ml', 'DQN_df_075_18_nodes_ml']
names = ['DQN_60_low_actions']
...
for i in range(len(names)):
    ...
    if i==1:  # optimise learning rate
        chosen_agent = agent(policies, env, verbose=1, learning_rate = 0.01, buffer_size=10000, batch_size=1000)
    if i==2:  # optimise the discount factor
        chosen_agent = agent(policies, env, verbose=1, gamma = 0.75, buffer_size=10000, batch_size=1000)
    else:  # standard case
        chosen_agent = agent(policies, env, verbose=1, gamma = 0.9995, buffer_size=10000, batch_size=1000)
